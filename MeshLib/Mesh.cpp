...
void Mesh::removePropertyVec(std::string const& prop_name)
{
...
std::vector<std::string> Mesh::getPropertyVecNames() const
{
	std::vector<std::string> names;
	names.resize(_prop_vecs.size());
	std::transform(_prop_vecs.begin(), _prop_vecs.end(),
			std::back_inserter(names),
			[](std::pair<std::string, std::vector<boost::any> > const& p) {
				return p.first;
			}
	);
	return names;
