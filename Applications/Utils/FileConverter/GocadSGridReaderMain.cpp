...
#include "FileIO/GocadIO/GocadSGridReader.h"
	...
	std::string const& path)
{
	...
	MeshLib::Properties &prop(new_mesh.getProperties());
	boost::optional<MeshLib::PropertyVector<unsigned>> regions_opt(
		prop.createNewPropertyVector<unsigned>("Regions", MeshLib::MeshItemType::Cell));
	if (regions_opt) {
		MeshLib::PropertyVector<unsigned> & regions_prop(*regions_opt);
		regions_prop.resize(regions.size());
		std::copy(regions.cbegin(), regions.cend(), regions_prop.begin());
	}

	FileIO::VtuInterface vtu(&new_mesh);
	// output file name
...
void generateFaceSetMeshes(FileIO::GocadSGridReader const& reader, std::string const& path)
{
	for (std::size_t l(0); l<128; l++) {
		...
		FileIO::VtuInterface vtu(face_set_mesh);
		// output file name
...
void addGocadPropertiesToMesh(FileIO::GocadSGridReader const& reader, MeshLib::Mesh &mesh)
{
	...
	for (auto name_it(prop_names.begin()); name_it != prop_names.end(); name_it++) {
		boost::optional<FileIO::GocadSGridReader::GocadProperty const&> prop(
			reader.getProperty(*name_it));
		if (!prop)
			continue;
...
//		mesh.addPropertyVec(*name_it, reworked_properties);
		...
		boost::optional<MeshLib::PropertyVector<double>> opt_pv(
			properties.createNewPropertyVector<double>(
				*name_it, MeshLib::MeshItemType::Cell));
		if (opt_pv) {
			MeshLib::PropertyVector<double> & property_vec(*opt_pv);
			property_vec.resize(prop->_property_data.size());
			std::copy(prop->_property_data.cbegin(),
				prop->_property_data.cend(), property_vec.begin());
		}
	}
}
...
{
	boost::optional<MeshLib::PropertyVector<double> const&> prop_vec(
