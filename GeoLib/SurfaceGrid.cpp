	...
	AABB(sfc->getAABB()), _n_steps({{1,1,1}})
{
	...
	switch (dim.count()) {
	case 3: // 3d case
		_n_steps[0] = ceil(std::cbrt(
			n_tris*delta[0]*delta[0]/(n_tris_per_cell*delta[1]*delta[2])));
		_n_steps[1] = ceil(_n_steps[0] * std::min(delta[1] / delta[0], 100.0));
		_n_steps[2] = ceil(_n_steps[0] * std::min(delta[2] / delta[0], 100.0));
		break;
	case 2: // 2d cases
		if (dim[0] && dim[2]) { // 2d case: xz plane, y = const
			_n_steps[0] = ceil(std::sqrt(n_tris*delta[0]/(n_tris_per_cell*delta[2])));
			_n_steps[2] = ceil(_n_steps[0]*delta[2]/delta[0]);
		}
		else if (dim[0] && dim[1]) { // 2d case: xy plane, z = const
			_n_steps[0] = ceil(std::sqrt(n_tris*delta[0]/(n_tris_per_cell*delta[1])));
			_n_steps[1] = ceil(_n_steps[0] * delta[1]/delta[0]);
		}
		else if (dim[1] && dim[2]) { // 2d case: yz plane, x = const
			_n_steps[1] = ceil(std::sqrt(n_tris*delta[1]/(n_tris_per_cell*delta[2])));
			_n_steps[2] = ceil(n_tris * delta[2] / (n_tris_per_cell*delta[1]));
		}
		break;
	case 1: // 1d cases
		for (std::size_t k(0); k<3; ++k) {
			if (dim[k]) {
				_n_steps[k] = ceil(n_tris/n_tris_per_cell);
			}
		}
	}
